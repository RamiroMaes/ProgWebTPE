...
echo
curl -sS -H "$JSON" --data '{"tipo_lesion":"Contractura","descripcion":"Muslo derecho"}' "$BASE_URL/lesiones"
echo
curl -sS -H "$JSON" --data '{"tipo_lesion":"Rotura fibrilar","descripcion":"Gemelo izquierdo"}' "$BASE_URL/lesiones"
echo
curl -sS -H "$JSON" --data '{"tipo_lesion":"Fatiga muscular","descripcion":"General"}' "$BASE_URL/lesiones"
echo
curl -sS "$BASE_URL/lesiones"
echo

printf "\n sentencias de club \n"
curl -sS -H "$JSON" --data '{"nombre":"Racing","ciudad":"Avellaneda"}' "$BASE_URL/clubs"
echo
curl -sS -H "$JSON" --data '{"nombre":"San Lorenzo","ciudad":"Buenos Aires"}' "$BASE_URL/clubs"
echo
curl -sS -H "$JSON" --data '{"nombre":"Independiente","ciudad":"Avellaneda"}' "$BASE_URL/clubs"
echo
curl -sS "$BASE_URL/clubs"
echo
curl -sS "$BASE_URL/clubs/Racing/Avellaneda"
echo

printf "\n actualizamos club Racing \n"
curl -sS -X PUT -H "$JSON" --data '{"nombre":"Racing Club","ciudad":"Avellaneda","nombre_2":"Racing","ciudad_2":"Avellaneda"}' \
  "$BASE_URL/clubs/Racing/Avellaneda"
echo
